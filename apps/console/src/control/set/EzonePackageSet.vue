<template>
    <div class="set_title">
        简单云（制品库）集成配置
        <el-button v-if="normalTeamAuth||isUserSpace" @click="$router.push('/detect-access?from=ezone_package')">新增集成</el-button>
    </div>
    ...
    <div class="config_item" v-for="(item, index) in  configList " :key="index">
        <div class="name">
            ...
            <span class="to-detect" @click="$router.push(`/detect-main?from=ezone_package&config_id=${item.config_id}`)">去检测 <i
...
onBeforeMount(() => {
    configList.value = store.allAccessInfo.ezone_package || [];
...
const changeHandle = (item: any) => {
    ...
    item.access_type = 'ezone_package';
};
...
const cancleHandle = async (item: any, index: number) => {
    ...
    const ezonePackageItem = data.ezone_package[index];
    item.status = 'save';
    item.config_name = ezonePackageItem.config_name;
    item.server_addr = ezonePackageItem.server_addr;
    item.server_token = ezonePackageItem.server_token;
    item.server_username = ezonePackageItem.server_username;
};
