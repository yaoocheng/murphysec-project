    ...
    <div class="table_wrap">
        <el-table :data="listData" height="100%" :cell-style="{ height: '48px' }"
            ...
            <el-table-column v-if="userRole !== 'SAU'" fixed="right" label="操作" width="60">
                <template #default="scope">
                    <span class="g-mr-8 g-color-5 g-cursor-pointer button-text-blue"
                          @click="editData = scope.row; isAddModal = true">查看</span>
            ...
            <el-popconfirm popper-class="cancle-popper confirm-popper" @confirm="deleteStrategyList([scope.row.id])"
                :title="`确认删除?`">
                <template #reference>
