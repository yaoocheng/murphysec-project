<template>
    <div class="set_title">
        Harbor集成配置
        <el-button v-if="normalTeamAuth||isUserSpace" @click="$router.push('/detect-access?from=harbor')">新增集成</el-button>
    </div>
    ...
    <div class="config_item" v-for="(item, index) in configList" :key="index">
        <div class="name">
            ...
            <span class="to-detect" @click="$router.push(`/detect-main?from=harbor&config_id=${item.config_id}`)">去检测 <i class="murphy icon-jiantou-you"></i></span>
        </div>
        <div class="config_content">
            <div v-if="item.status === 'save'">
                ...
                <div class="form_item">
                    ...
                    <div>
                        {{ versionOptions.find(ele => ele.data === item.versions)?.label }}
                    </div>
                </div>
            </div>
            ...
            <div v-if="item.status === 'edit'">
                <FormComp label-position="top" label-width="135px">
                    ...
                    <el-form-item prop="address" required>
                        ...
                        <el-select style="width: 500px" v-model="item.versions">
                            <el-option v-for="(ele, index) in versionOptions" :key="index" :label="ele.label"
                                       :value="ele.data"></el-option>
                        </el-select>
                    </el-form-item>
...
onBeforeMount(() => {
    configList.value = store.allAccessInfo.harbor || [];
...
const cancleHandle = async (item: any, index: number) => {
    ...
    item.status = 'save';
    Object.assign(item, res.data.harbor[index]);
};
