<template>
    <div class="set_title">
        Bitbucket集成配置
        <el-button v-if="normalTeamAuth||isUserSpace" @click="$router.push('/detect-access?from=bitbucket')">新增集成</el-button>
    </div>
    ...
    <div class="config_item" v-for="(item, index) in configList" :key="index">
        <div class="name">
            ...
            <span class="to-detect" @click="$router.push(`/detect-main?from=bitbucket&config_id=${item.config_id}`)">去检测 <i class="murphy icon-jiantou-you"></i></span>
        </div>
...
onBeforeMount(() => {
    configList.value = store.allAccessInfo.bitbucket || [];
...
const cancleHandle = async (item: any, index: number) => {
    ...
    item.status = 'save';
    Object.assign(item, res.data.bitbucket[index]);
};
