<template>
    <div class="set_title">
        Amazon ECR集成配置
        <el-button v-if="normalTeamAuth||isUserSpace" @click="$router.push('/detect-access?from=ecr')">新增集成</el-button>
    </div>
    ...
    <div class="config_item" v-for="(item, index) in configList" :key="index">
        <div class="name">
            ...
            <span class="to-detect" @click="$router.push(`/detect-main?from=ecr&config_id=${item.config_id}`)">去检测 <i class="murphy icon-jiantou-you"></i></span>
        </div>
        <div class="config_content">
            <div v-if="item.status === 'save'">
                <div class="form_item">
                    ...
                    <div>
                        {{ item.server_username ? '访问密钥认证' : 'ECR 实例角色认证' }}
                    </div>
                </div>
                <div class="form_item" v-if="item.server_username">
                    <div class="form_label">
                        身份验证密钥 ID
                    </div>
                    <div>
            ...
            <div v-if="item.status === 'edit'">
                <FormComp label-position="top" label-width="135px">
                    ...
                    <el-form-item prop="address" v-if="item.server_username">
                        <template #label>
                            <div class="form_label">
                                身份验证密钥 ID
                            </div>
                        ...
                        <div style="width: 500px">
                            <el-input v-model="item.server_username" clearable placeholder="请输入仓库Access key ID" />
                        </div>
                    </el-form-item>

                    <el-form-item prop="token" v-if="item.server_username">
                        <template #label>
                            <div class="form_label">
                                秘密访问密钥
                            </div>
                        ...
                        <div style="width: 500px">
                            <el-input v-model="item.server_password" clearable placeholder="请输入仓库Secret access key" />
                        </div>
                    </el-form-item>
...
onBeforeMount(() => {
    configList.value = store.allAccessInfo.ecr || [];
...
const cancleHandle = async (item: any, index: number) => {
    ...
    item.status = 'save';
    Object.assign(item, res.data.ecr[index]);
};
