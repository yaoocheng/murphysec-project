<template>
    ...
    <div class="g-p-24 g-bdr-6 g-bg-ff g-f-14 g-fw-400 g-mb-24">
        <div class="filter_wrap">
            <SearchComp style="width: 200px;" clearable v-model="filterOptions.asset_name" placeholder="请搜索组件名称" />
            ...
            <SelectComp style="width: 140px;" clearable v-model="filterOptions.access_type" :options="accessOptions"
                        placeholder="接入方式" />
            ...
            <SelectComp style="width: 140px;" v-model="filterOptions.is_online" :options="onlineOptions"
                        placeholder="线上依赖" />
            <SelectComp style="width: 140px;" v-model="filterOptions.is_poison" :options="posionOptions"
                        placeholder="投毒组件" />
        </div>
        <div @click=" bus.emit('showAdvanData', filterOptions); isShowFilterModal = true;"
             class="g-mb-24 advanced-filter">
            <div class="button-text-blue">
                <!-- <i class="murphy icon-lujing2 g-color-3 g-f-12 g-mr-4"></i> -->
                <i class="murphy icon-lujing2"></i>
                <span>高级筛选</span>
            </div>
            <span
                v-if="filterOptions.asset_list?.some(item => item.name) || filterOptions.project_name || filterOptions.vul_id"
                class="sqr">
                <p style="transform: scale(0.8);display: inline-block">
                    {{ filterOptions.asset_list.filter(item => item.name)?.length + (filterOptions.project_name ? 1 : 0)
                        +
                        (filterOptions.vul_id ? 1 : 0) }}
                </p>
            </span>
        </div>

        <div>
            <el-button @click="resetHandle" class="minor_btn" style="height: 32px;width: 80px;">重置</el-button>
            <el-button @click="createTaskHandle" class="main_btn" :disabled="disabled"
                       style="height: 32px;width: 80px;margin-left: 12px;">查询</el-button>
        </div>
    </div>
    ...
    <div class="g-bdr-6 g-p-16 g-bg-ff g-color-1 task-content">
        <div class="g-mt-40 g-ta-c g-mb-40" v-if="newQueryData?.id === '0'">
            <NoData description="暂无我的查询任务" :image-size="120" />
            <p class="no-data-desc" style="margin-bottom: 0;">可通过设置<span class="g-fw-500">「查询条件」</span>进行查询，查看组件最新的数据</p>
        ...
        <div class="g-ta-c g-mt-40 g-mb-40" v-else-if="newQueryData?.status === 1">
            ...
            <div class="g-color-3 g-fw-400 g-f-14 g-mb-8 g-flex g-align-center g-justify-center">
                ...
                <span class="hover_icon g-ml-4" @click="changeNameDialog = true">
        ...
        <div class="g-ta-c g-mt-40 g-mb-40" v-else-if="newQueryData?.status === 2">
            <div class="g-f-16 g-color-1 g-fw-500 g-mb-8 g-flex g-align-center g-justify-center">
                任务名称（最新）：{{ newQueryData?.project_name }}
                <span class="hover_icon g-ml-8" style="padding: 3px 5px" @click="changeNameDialog = true">
                    <i class="murphy icon-bianji g-cursor-pointer g-color-1 g-fw-500"></i>
                </span>
                ...
                <el-tooltip v-else placement="right" popper-class="user_tooltip" effect="light">
                    ...
                    <span class="hover_btn1 g-ml-12" style="padding: 4px 6px;">
                        <span class="g-color-5 g-fw-400 g-f-14 g-cursor-pointer">查询条件</span>
                    </span>
                </el-tooltip>
            </div>
            ...
            <div>
                ...
                <el-button
                    @click="showRelustModal = true; taskInfoId = newQueryData.id; taskInfo.name = newQueryData.project_name;
                            taskInfo.createTime = newQueryData.created_at">
                    查看结果
                </el-button>
            </div>
        </div>
        ...
        <div style="margin-top: -12px;">
            <div class="g-flex g-justify-between g-align-center">
                ...
                <div class="g-flex g-align-center">
                    ...
                    <span @click="getData()" class="button-icon-blue">
    ...
    <AdvancedFilterModal v-model="isShowFilterModal" @confirmFilterHandle="confirmFilterHandle" />

    <AssetResultModal v-model="showRelustModal" v-if="showRelustModal" :id="taskInfoId" :taskInfo="taskInfo" />
</template>
...
import {
    createAssetsTaskApi, getAssetsTaskApi, cancelQueryApi, deleteExportApi, getTaskInfoApi, getNewItemApi, reNameApi,
} from 'api/asset';
...
import {
    repairOptions, accessOptions, triggerOptions, posionOptions, onlineOptions, langOptions,
} from './constants';
import AdvancedFilterModal from './AdvancedFilterModal.vue';
import AssetResultModal from './AssetResultModal.vue';
import bus, { formatDate, showMessage } from '@/utils/tools';

const queryModal = ref(false); // 查询结果
    ...
    try {
        ...
        if (newQueryData.value?.status === 2) {
            try {
                ...
                const { data } = await getTaskInfoApi({ id: newQueryData.value.id });
...
// 高级筛选
const confirmFilterHandle = (data: any) => {
    Object.assign(filterOptions.value, data);
};
