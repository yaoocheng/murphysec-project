<template>
    <div ref="fileCodeRef" class="flex relative overflow-auto box-border h-full">
        <!-- 蒙层 -->
        <div ref="hide" :class="{ 'active_frag': fragCodeMatchFileId === item.source_id }"
            :id="`match_code_${item.source_id}`" @click.stop="fragCodeMatchFileId = item.source_id;"
...
import { ITEM_CODE_HEIGHT, fragCodeMatchFileId } from './business';
...
watchEffect(() => {
    const dom = document.getElementById(`match_code_${fragCodeMatchFileId.value}`);
    if (dom) {
        dom.scrollIntoView(true);
    } else {
        nextTick(() => {
            const dom1 = document.getElementById(`match_code_${fragCodeMatchFileId.value}`);
            if (dom1) {
                dom1.scrollIntoView(true);
            }
        });
    }
});
