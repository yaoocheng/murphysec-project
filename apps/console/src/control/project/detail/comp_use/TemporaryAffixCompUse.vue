<template>
    <el-tooltip v-if="temporaryStorageUseComp.length > 0" append-to=".affix" :visible="visible" placement="top-end"
        effect="light">
        <template #content>
            <div @mouseleave="visible = false;" class="w-[344px] p-2 text-sm flex items-center">
                「组件用法校准」信息会被暂时保存在此，点击可查看，应用后即生效
            </div>
        </template>
        <div ref="affixRef" @click="openAppAdjustModal" @mouseenter="visible = true;" @mouseleave="visible = false;"
            ...
            <TagComp class="rounded-xl text-xs leading-[13px] scale-75" :content="temporaryStorageUseComp.length" c="#fff" bgc="#F34D3D" />
        </div>
    </el-tooltip>
</template>
...
import { isShowAppAdjustUseModal, temporaryStorageUseComp } from './business';
...
watch(temporaryStorageUseComp, (newVal) => {
...
const openAppAdjustModal = () => {
    if (isClick.value) {
        isShowAppAdjustUseModal.value = true;
    }
};
</script>
