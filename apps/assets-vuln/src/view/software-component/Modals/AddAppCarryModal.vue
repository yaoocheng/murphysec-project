<template>
    <a-modal class="add-app-carry-modal" :confirmLoading="globalState.loading" title="新增应用载体" :afterClose="clearFormHandle"
        ...
        <div style="height: calc(100vh - 240px);overflow: auto;" ref="modalContentRef">
            <div class="sticky top-0 z-10 bg-white mb-4">
                <a-anchor class="anchor" direction="horizontal" :target-offset="40" :get-container="() => modalContentRef" @click="handleClickAnchor">
            ...
            <div class="flex flex-col gap-10">
                ...
                <div class="flex flex-col gap-4" id="detailInfo">
                    ...
                    <a-form autocomplete="off" v-if="formState.baseInfo.carrier_type === 'code_repo'"  class="base-form bg-[#F2F2F280] rounded-md p-6 gap-4 grid grid-cols-2" ref="formRef" :model="formState.code_repo_detailInfo" layout="vertical">
                        ...
                        <a-form-item label="仓库URL" name="repo_url" :rules="[{ required: true, message: '必填项不能为空' }]">
                            <template #label>
                                仓库URL
                            </template>
                    ...
                    <a-form  autocomplete="off" v-else class="base-form bg-[#F2F2F280] rounded-md p-6 gap-4 grid grid-cols-2" ref="formRef" :model="formState.service_address_detailInfo" layout="vertical">
                        <a-form-item label="协议类型" name="protocol" :rules="[{ required: true, message: '必填项不能为空' }]">
                            <template #label>
                                协议类型
                            </template>
                        ...
                        <a-form-item label="IP" name="ip" :rules="[{ required: true, message: '必填项不能为空' }]">
                            <template #label>
                                IP
                            </template>
                        ...
                        <a-form-item label="端口号" name="port" :rules="[{ required: true, message: '必填项不能为空' }]">
                            <template #label>
                                端口号
                            </template>
...
const okBtndisabled = computed(() => {
    // 基本必填项验证
    const baseInfoValid = !formState.baseInfo.name || !formState.baseInfo.carrier_type || formState.application_id.length === 0;

    // 根据载体类型验证不同的必填项
    if (formState.baseInfo.carrier_type === 'code_repo') {
        // 代码仓库类型：仓库URL为必填项
        return baseInfoValid || !formState.code_repo_detailInfo.repo_url;
    }
    // 服务地址类型：协议类型、IP、端口为必填项
    return baseInfoValid || !formState.service_address_detailInfo.protocol || !formState.service_address_detailInfo.ip || !formState.service_address_detailInfo.port;
});
