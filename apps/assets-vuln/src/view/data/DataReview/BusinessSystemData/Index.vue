    ...
    <div class="h-full flex flex-col">
        <div style="border-bottom: 8px solid #f2f2f2;"
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-2">
                    <Input v-model:value="filters.business_system_name" placeholder="搜索业务系统名称"
                           class="w-[240px] base-input" />

                    <Select :popupClassName="'base-select-dropdown'" class=" base-select"
                            v-model:value="filters.business_system_status" :options="SYSTEM_STATUS_OPTION"
                            placeholder="业务系统状态" />
                    <Select :popupClassName="'base-select-dropdown'" class=" base-select"
        ...
        <div class="flex-1 px-[24px] py-[16px] gap-[24px] flex flex-col">
            <a-table :row-selection="rowSelection" class="base-table-select" :pagination="false"
                     ...
                     :scroll="{ x: '100%', y: 'calc(100vh - 320px)' }" :data-source="listData" rowKey="business_system_id">
                <template #emptyText>
                    <Empty />
                </template>
                ...
                <template #bodyCell="{ column, index, text, record }">
                    ...
                    <template v-if="column.dataIndex === 'issue_title' || column.dataIndex === 'business_system_name'
                        ...
                        <a-tooltip arrowPointAtCenter>
                            ...
                            <span v-if="column.dataIndex === 'issue_title'" @click="RouteJump(index, record.issue_id)"
                                  class="ellipsis hover w-full">
                                {{ text }}
                            </span>
                            <span class="w-full truncate" v-else>
                                {{ text || '-' }}
                            </span>
                        </a-tooltip>
                    </template>

                    <template v-else-if="column.dataIndex === 'business_system_status'">
                        <span>{{ findApplicationStatusLabel(text) }}</span>
                    </template>
                    ...
                    <template v-else-if="column.dataIndex === 'updated_at' || column.dataIndex === 'discovery_at' || column.dataIndex === 'created_at'">
                        {{ formatDate(text) }}
                    </template>
                    ...
                    <template v-else-if="column.dataIndex === 'operation'">
                        <div class="flex gap-4 items-center">
                            ...
                            <a-popover :overlayStyle="{ zIndex: 500 }"
                                ...
                                <template #content>
                                    <div class="cursor-pointer">
                                        <div @click="turnOnLog(record.business_system_id)"
        ...
        <Drawer v-if="opendrawer" v-model:open="opendrawer" :listData="listData" @getData="getData();selectedSecIssue=[];selectBulkData=[]"
                @handleRelease="handleRelease" :clickindex="clickindex" />
    </div>
</template>

<script setup>
import {
    ref, reactive, onBeforeMount, h,
    computed,
} from 'vue';
import {
    getDataPreparationBusinessSystemListApi, delDataPreparationBusinessSystemApi, pushDataPreparationBusinessSystemApi, batchDelPluginApi, batchPublishPluginApi,
} from 'api/data';
import {
    indicator, formatDate, resetFilter, emitter,
} from 'utils';
import {
    SYSTEM_STATUS_OPTION, getSecurityCapabilityList, findApplicationStatusLabel,
} from 'constants/app';
import { Modal, message } from 'ant-design-vue';
import { findDataStatusLabel, DATA_STATUS_OPTIONS } from '../../constants/datareview';
...
const filters = reactive({
    limit: 15,
    business_system_name: null,
    business_system_status: null,
    data_status: null,
...
const turnOnLog = (business_system_id) => {
    console.log('turnOnLog', business_system_id);
...
} = usePageList(getDataPreparationBusinessSystemListApi, filters);

const selectedSecIssue = ref([]); // 选中的行数据
const selectBulkData = ref([]); // 能批量选中的数据

const selectedRowKeys = computed(() => selectedSecIssue.value.map((item) => item.business_system_id)); // 处理回显
const rowSelection = {
    selectedRowKeys,
    ...
    onSelect: (record, selected, selectedRows) => {
        if (selected) {
            ...
            selectedSecIssue.value = selectedSecIssue.value.filter((item) => item.business_system_id !== record.business_system_id);
            selectBulkData.value = selectBulkData.value.filter((item) => item.business_system_id !== record.business_system_id);
        }
    ...
    onSelectAll: (selected, selectedRows) => {
        ...
        if (selected) {
            ...
            selectedSecIssue.value = selectedSecIssue.value.filter((item) => !listData.value.some((listItem) => listItem.business_system_id === item.business_system_id));

            const curList = listData.value.filter((item) => item.data_status === 'perfect');
            selectBulkData.value = selectBulkData.value.filter((item) => !curList.some((listItem) => listItem.business_system_id === item.business_system_id));
        }
    },
...
const bulkRelease = () => {
    Modal.confirm({
        ...
        async onOk() {
            try {
                const { data } = await pushDataPreparationBusinessSystemApi({
                    business_system_ids: selectBulkData.value.map((item) => item.business_system_id),
                });
...
const columns = ref([
    ...
    {
        title: '级别',
        dataIndex: 'business_system_level', // 修改
        width: '80px',
    },
    {
        title: '上级业务系统',
        dataIndex: 'parent_business_system', // 修改
        width: '240px',
    },
    {
        title: '业务系统状态',
        dataIndex: 'business_system_status', // 修改
        width: '152px',
    },
    {
        title: '业务系统负责人',
        dataIndex: 'business_system_owner_nick_name', // 修改
        width: '178px',
        type: 'person',
    },
    {
    ...
    {
        title: '数据状态',
        dataIndex: 'data_status', // 修改
        width: '132px',
    },
    {
        title: '数据处理人',
        dataIndex: 'data_processor_name', // 修改
        width: '178px ',
        type: 'person',
    },
    {
        title: '数据接入时间',
        dataIndex: 'created_at', // 修改
        width: '178px',
    },
    {
        title: '数据更新时间',
        dataIndex: 'updated_at', // 修改
        width: '178px',
    },
    {
        title: '操作',
        ...
        async onOk() {
            try {
                await pushDataPreparationBusinessSystemApi({
                    business_system_ids: [record.business_system_id],
                });
            } catch {
                return console.log('Oops errors!');
            } finally {
...
const handledelete = (record) => {
    Modal.confirm({
        ...
        async onOk() {
            try {
                await delDataPreparationBusinessSystemApi([record.business_system_id]);
            } catch {
                return console.log('Oops errors!');
            } finally {
                getData();
...
const bulkDelete = () => {
    showDeleteConfirm({
        record: selectedSecIssue.value.map((item) => item.business_system_id),
        onDelete: delDataPreparationBusinessSystemApi,
        onSuccess: (data) => {
            getData();
            selectedSecIssue.value = [];
            selectBulkData.value = [];
...
const fullReleaseHandle = () => {
    Modal.confirm({
        ...
        async onOk() {
            try {
                const { data } = await batchPublishPluginApi('business_system');
                message.success(`成功发布 ${data.data.success_count} 条数据，失败 ${data.data.fail_count} 条数据`);
            } catch {
                return console.log('Oops errors!');
            } finally {
